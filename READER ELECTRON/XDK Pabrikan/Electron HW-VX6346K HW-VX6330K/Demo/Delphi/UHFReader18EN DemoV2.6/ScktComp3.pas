...
type
  ...
  public
    ...
    procedure SetLocalPort(Value: integer;ClientNum:longint);
    ...
    property Addr: TSockAddrIn read FAddr write FAddr;
  ...
  public
    ...
    destructor Destroy; override;
    property Socket: TClientWinSocket read FClientSocket write FClientSocket;
  published
    property Active;
    property Address;
...
var
  WSAData: TWSAData;
  FSockAddrIn:array [1..65535] of TSockAddrIn;
...
begin
  ...
  SockAddrIn.sin_port:=FAddr.sin_port;
  SockAddrIn.sin_family:=FAddr.sin_family;
  SockAddrIn.sin_addr.S_addr:=FAddr.sin_addr.S_addr;
  if  SockAddrIn.sin_port >=1 then
  FAddr.sin_port:=FSockAddrIn[ntohs(FAddr.sin_port)].sin_port;
  FAddr.sin_family:=PF_INET;
...
//    bind(FSocket, FAddr, Sizeof(FAddr));
  BON:=1;
  BOO:=true;
//  setsockopt(FSocket, SOL_SOCKET, SO_REUSEADDR, PChar(@BON), sizeof(BON));
   setsockopt(FSocket, SOL_SOCKET, SO_REUSEADDR, PChar(@BOO), sizeof(BOO));
//   bDontLinger:= FALSE;
//   setsockopt(FSocket,SOL_SOCKET,SO_DONTLINGER,PChar(@bDontLinger), sizeof(bDontLinger));
//  FAddr.sin_family := AF_INET;
//  FAddr.sin_addr.s_addr := htonl( INADDR_ANY );
//  memset( &servaddr, 0, sizeof(servaddr) );
  CheckSocketResult(bind(FSocket, FAddr, SizeOf(FAddr)), 'bind');
  FSocketLock.Leave;
  FAddr.sin_port:=SockAddrIn.sin_port;
  FAddr.sin_family:=SockAddrIn.sin_family;
  FAddr.sin_addr.S_addr:=SockAddrIn.sin_addr.S_addr;
...
procedure TCustomWinSocket.SetLocalPort(Value: integer;ClientNum:longint);
begin
  Lock;
  try
    FSockAddrIn[ClientNum].sin_port   := htons(Value);

   // if bind(FSocket, SockAddrIn, Size) = -1 then
  //    Result := false
  //  else
 //     Result := True;
  finally
...
{    bDontLinger:= FALSE;
    setsockopt(FSocket,SOL_SOCKET,SO_DONTLINGER,PChar(@bDontLinger), sizeof(boolean));
    linger.l_onoff:=0;//(在closesocket()调用,但是还有数据没发送完毕的时候容许逗留)
    // 如果m_sLinger.l_onoff=0;则功能和2.)作用相同;
    linger.l_linger:=1;//(容许逗留的时间为5秒)
    setsockopt(FSocket,SOL_SOCKET,SO_LINGER,PChar(@linger), sizeof(linger));  }
    ...
    FAddr.sin_addr.s_addr := INADDR_ANY;
 //   FAddr.sin_port := 0;
//    FAddr.sin_port:=FSockAddrIn.sin_port;
    FConnected := False;
